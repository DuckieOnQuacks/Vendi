...
class FirebaseHelper {
  ...
  Future<List<Machine>> getFilteredMachines(
    ...
    if (snack) {
      final snackSnapshot = await machinesCollection
          .where('icon', isEqualTo: "assets/images/BlueMachine.png")
          .get();
      filterDocs.addAll(snackSnapshot.docs);
    }
    if (drink) {
      final drinkSnapshot = await machinesCollection
          .where('icon', isEqualTo: "assets/images/PinkMachine.png")
          .get();
      filterDocs.addAll(drinkSnapshot.docs);
    }
    if (supply) {
      final supplySnapshot = await machinesCollection
          .where('icon', isEqualTo: "assets/images/YellowMachine.png")
          .get();
